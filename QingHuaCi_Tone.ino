...
void loop() {
  ...
  int thisPitch = map(sensorValue, 0, 1024, 0, 2048);
  ...
  if ((millis() - lastDebounceTime) > debounceDelay) {
    ...
    if (reading != buttonState) {
      ...
      if (buttonState == HIGH) {
        ...
        if(melodyCounter < melodyMaximum){
          melodyRecord[melodyCounter++] = thisPitch;
//          Serial.println(thisPitch);
          for (int i=0; i<melodyCounter; i++){
            Serial.print(melodyRecord[i]);
            Serial.print(" ");
          }
          Serial.println();
          delay(10);
  ...
  if ((millis() - lastDebounceTime) > 3000) {
    for (int thisNote = 0; thisNote < 8; thisNote++) {
      ...
      tone(piezoPin, melodyRecord[thisNote], noteDuration);  //melody[thisNote]
